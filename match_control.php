...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    switch ($action) {
        case 'start_match':
            ...
            $result = executeRconCommand($match['server_ip'], 'matchzy_loadmatch_url "' . $configUrl . '"');
            
            // Atualizar status
            updateMatchStatus($match['match_id'], 'loading');
            
            // Mensagem de sucesso com URL
            if ($result['success']) {
                $message = 'Comando de iniciar partida enviado com sucesso!<br>Config URL: <a href="' . $configUrl . '" target="_blank" class="text-blue-300 hover:text-blue-200">' . $configUrl . '</a>';
            } else {
                $message = 'Erro ao enviar comando: ' . ($result['message'] ?? 'Erro desconhecido');
            }
            break;
